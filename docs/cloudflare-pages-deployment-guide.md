...
> 2026-09-22 本次发布：在上一轮已告知人工审核仍待完成后，项目负责人明确要求“如果没有其他问题的话，就提交上线吧”。本次按该授权提交技术检查通过的版本，沿用现有 GitHub → Cloudflare Pages 流程；不修改待审登记，不宣称真人审校已通过。详细记录见 `docs/releases/2026-09-22.md`。下方审核流程说明保留为默认流程，本次是有记录的发布授权例外。

> 2026-09-22 更新：下文保留原有部署操作记录。目前新文案尚未完成人工审核，不能把 `npm run check` 通过当成内容可发布。新的正式发布命令为 `npm run build:release`，会核对人工审核登记与内容版本；当前有待审项目，预期不会通过。Cloudflare 控制台的远程构建命令尚未修改，旧的 `npm run build` 自动部署流程不会执行此审核检查。完成真实审核并更新生产构建设置后，才应推送本批内容。详见 `docs/内容研究/研究结论与下一步.md` 和 README 的研究审核说明。
